...
type FreelanceService = {
  ...
  state?: string; // now stores abbreviation, e.g. "TX"
  city?: string;
  rate?: string;
  email?: string;
  website?: string;
...
export default function FreelancePage() {
  ...
  const [filterState, setFilterState] = useState<string>(""); // abbreviation string
  ...
  async function fetchFilterCities(stateAbbr: string) {
    try {
      const res = await apiFetch(`/us-cities?state=${encodeURIComponent(stateAbbr)}`);
  ...
  async function fetchCities(stateAbbr: string, currentCity: string) {
    try {
      const res = await apiFetch(`/us-cities?state=${encodeURIComponent(stateAbbr)}`);
      if (!res.ok) throw new Error("Failed to fetch cities");
      const data = await res.json();
  ...
  // Helper: get state full name from abbreviation
  ...
  const filteredServices = services.filter((service) => {
    const matchesServiceType = filterServiceType
      ? service.service_type === filterServiceType
      : true;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredServices.map((service) => (
            ...
            >
              ...
              <p className="text-sm text-gray-500 mb-1">
                Location: {getStateName(service.state || "")}{" "}
              ...
              <p className="text-sm text-gray-500 mb-1">
                ...
                {service.other_service && service.other_service.trim() !== ""
                  ...
                  : serviceOptions.find((s) => s.id === Number(service.service_type))?.name ||
                    "N/A"}
              </p>
        ...
        {modalService && (
          ...
          >
            ...
            >
              ...
              <p className="mb-2">
                <strong>Description:</strong> {modalService.description || "N/A"}
              </p>
              ...
              <p className="mb-2">
                <strong>Location:</strong> {getStateName(modalService.state || "")}{" "}
                {modalService.city ? `, ${modalService.city}` : ""}
              </p>
              <p className="mb-2">
                <strong>Service Type:</strong>{" "}
                {serviceOptions.find((s) => s.id === Number(modalService.service_type))?.name ||
                  "N/A"}
              </p>
