...
export default function StudentCheckoutSuccess() {
  ...
  const confirmPayment = async () => {
    if (!sessionId || !token) {
      setError("Session ID or token missing.");
      setLoading(false);
      return;
    }
  ...
  if (loading) {
    return <p>Processing your payment...</p>; // Show loading message while confirming payment
  }

  if (error) {
    return <p className="text-red-600">{error}</p>; // Show error message if there was any issue
  }
