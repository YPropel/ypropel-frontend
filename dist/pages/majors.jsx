...
function Majors() {
    const sortedMajors = [...mockMajors].sort((a, b) => a.title.localeCompare(b.title));
    ...
    <div className="max-w-7xl mx-auto p-6 bg-white rounded shadow space-y-6">
      ...
      <div className="flex flex-col space-y-6">
        {sortedMajors.map((major) => (<div key={major.id} className="flex flex-col md:flex-row border rounded shadow hover:shadow-lg transition p-4">
            {/* Details Left */}
            <div className="flex-1 pr-4">
              ...
              {major.popularUniversities.length > 0 && (<div>
                  <p className="font-semibold text-gray-700 mb-1">Popular Universities:</p>
                  <ul className="list-disc list-inside text-gray-600">
                    {major.popularUniversities.map((uni, idx) => (<li key={idx}>{uni}</li>))}
                  </ul>
                </div>)}
              <a href="#" className="mt-4 inline-block text-blue-700 font-semibold hover:underline">
                Learn More
              </a>
            </div>
            ...
            <div className="flex-shrink-0 mt-4 md:mt-0 md:ml-4 w-full md:w-48 h-32 rounded overflow-hidden">
              <img src={major.imageUrl} alt={`${major.title} image`} className="w-full h-full object-cover" loading="lazy"/>
            </div>
          </div>))}
      </div>
    </div>
