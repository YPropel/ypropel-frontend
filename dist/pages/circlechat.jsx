...
function CircleChat() {
    ...
    const [circleName, setCircleName] = (0, react_1.useState)("");
    ...
    (0, react_1.useEffect)(() => {
        ...
        const fetchCircleNameFromAll = async () => {
            ...
            const match = circles.find((c) => c.id === parseInt(id));
            if (match) {
    ...
    return (<div className="p-6 max-w-3xl mx-auto">
      {/* 🔙 Back Button */}
      <button onClick={goBackToCircles} className="mb-4 bg-gray-200 text-gray-800 px-4 py-1 rounded hover:bg-gray-300">
        ← Back to All Circles
      </button>
  ...
  Chat in {circleName ? `Circle: ${circleName}` : `Circle #${id}`}
      ...
      <div className="h-60 overflow-y-auto bg-gray-100 p-3 rounded mb-4">
        {messages.length === 0 ? (<p className="text-sm text-gray-500">No messages yet.</p>) : (messages.map((msg) => (<div key={msg.id} className="mb-2 text-sm text-gray-800">
    ...
    {msg.message}
      ...
      <div className="flex gap-2">
        ...
        <button onClick={sendMessageToCircle} className="bg-blue-900 text-white px-4 py-1 rounded hover:bg-blue-800">
          Send
        </button>
      </div>
    </div>);
